...
void initConnection(int Port, std::string IP){
	...
	// Создадим пару из указателя на наш вектор и указателя на критическую секцию

	DataForSharedMemory.Vector = &BoxOfMessages;
	DataForSharedMemory.CS = &G_CS_MES;

	VectorAndCS *ptrDataForSharedMemory = &DataForSharedMemory; // Делаем указатель на структуру чтобы его адрес кинуть в memcpy
	std::memcpy(region.get_address(), &ptrDataForSharedMemory, region.get_size()); // записали данные  в нашу расшаренную область памяти
