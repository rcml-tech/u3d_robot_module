...
*Например VS запущен на компьютере с ip 192.168.1.124, и слушает порт 12000, при этом мы хотим создать мир с границами 100,100,10 тогда config.ini будет иметь следующее содержимое:*
```ini
    [connection]
    ip = 192.168.1.124
    port = 12000
...
Переменная **color** должна принимать строковое значение в виде трёх пар шестнадцатеричных цифр(0-F), где каждая пара отвечает за красный, зеленый и синий цвета соответственно <br>
